...
This document describes the standard release process for zookeeper-operator.
...
4. **Helm chart publish** — pushes to
   `quay.io/kubedoopcharts/zookeeper-operator:<version>` (OCI registry)
...
```bash
gh api repos/zncdatadev/zookeeper-operator/git/refs \
  -f ref=refs/heads/release-0.x \
  -f sha=$(gh api repos/zncdatadev/zookeeper-operator/git/ref/heads/main --jq .object.sha)
```
...
zookeeper-operator follows [Semantic Versioning](https://semver.org/):
